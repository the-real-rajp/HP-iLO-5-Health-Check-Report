...
function Resolve-ReportLogoPath {
    ...
    $temporaryLogoPath = Join-Path ([IO.Path]::GetTempPath()) ('winslowtg-logo-' + [guid]::NewGuid().ToString('N') + '.png')
    ...
    try {
        ...
        if (Test-Path $temporaryLogoPath) {
            Remove-Item -LiteralPath $temporaryLogoPath -Force -ErrorAction SilentlyContinue
        }
        if (Test-Path $script:ReportLogoPath) {
            Write-Verbose "Unable to download the WinslowTG logo; using the bundled fallback at $script:ReportLogoPath"
...
function Get-ReportRecords {
    ...
    if ($PropertyMap -and $property.Value -is [Collections.IDictionary]) {
        return @($property.Value.GetEnumerator() | ForEach-Object {
            if ($_.Key -notin $ExcludeColumns -and -not (Test-UnknownReportValue $_.Value)) {
...
function Set-WordReportFurniture {
    ...
    foreach ($headerIndex in 1..3) {
        ...
        $header.Text = "`tHP iLO Health Check $([char]0x2013) $Target $([char]0x2013) $CustomerName"
        ...
        $logo = $header.InlineShapes.AddPicture($logoPath, $false, $true, $logoRange)
        $logo.Width = 105
        $logo.Height = 32.25
...
function Get-OpenXmlChildElement {
    param(
    ...
    return $Parent.SelectSingleNode("w:$Name", $NamespaceManager)
}

function Ensure-OpenXmlChildElement {
    param(
        [Parameter(Mandatory)][System.Xml.XmlNode]$Parent,
        [Parameter(Mandatory)][string]$Name,
    ...
    $child = Get-OpenXmlChildElement -Parent $Parent -Name $Name -NamespaceManager $NamespaceManager
    if ($null -eq $child) {
        $child = $Document.CreateElement('w', $Name, $NamespaceManager.LookupNamespace('w'))
        [void]$Parent.AppendChild($child)
    }
    return $child
}

function Set-OpenXmlAttribute {
    param(
    ...
    [void]$Element.SetAttribute($Name, $NamespaceManager.LookupNamespace('w'), $Value)
}

function Get-OpenXmlCellGridSpan {
    param(
        [Parameter(Mandatory)][System.Xml.XmlNode]$Cell,
        [Parameter(Mandatory)][System.Xml.XmlNamespaceManager]$NamespaceManager
    )
    ...
    if ($null -ne $gridSpan) {
        try {
            $span = [int]$gridSpan.GetAttribute('val', $NamespaceManager.LookupNamespace('w'))
            if ($span -gt 0) { return $span }
        }
        catch { }
    }
    return 1
...
function Get-OpenXmlDocumentContentWidth {
    param(
        [Parameter(Mandatory)][System.Xml.XmlDocument]$Document,
        [Parameter(Mandatory)][System.Xml.XmlNamespaceManager]$NamespaceManager
    )

    $sectionProperties = $Document.SelectSingleNode('//w:sectPr', $NamespaceManager)
    if ($null -eq $sectionProperties) { return 10800 }
    try {
        ...
        $pageWidth = [int]$pageSize.GetAttribute('w', $namespace)
        $leftMargin = [int]$pageMargins.GetAttribute('left', $namespace)
        $rightMargin = [int]$pageMargins.GetAttribute('right', $namespace)
        return [Math]::Max(1440, $pageWidth - $leftMargin - $rightMargin)
    }
    catch {
        return 10800
    }
...
function Get-OpenXmlContentBasedColumnWidths {
    param(
    ...
    foreach ($row in $rows) {
        ...
        foreach ($cell in @($row.SelectNodes('./w:tc', $NamespaceManager))) {
            $rowColumnCount += Get-OpenXmlCellGridSpan -Cell $cell -NamespaceManager $NamespaceManager
        }
        $columnCount = [Math]::Max($columnCount, $rowColumnCount)
    }
    if ($columnCount -eq 0) { return @() }

    $weights = New-Object 'System.Double[]' $columnCount
    for ($index = 0; $index -lt $columnCount; $index++) {
        $weights[$index] = [double]$MinimumWidth
    }
    foreach ($row in $rows) {
        $columnIndex = 0
        foreach ($cell in @($row.SelectNodes('./w:tc', $NamespaceManager))) {
            $span = Get-OpenXmlCellGridSpan -Cell $cell -NamespaceManager $NamespaceManager
            $text = (@($cell.SelectNodes('.//w:t', $NamespaceManager) | ForEach-Object { $_.InnerText.Trim() } | Where-Object { $_ }) -join ' ')
            $longestLine = 0
            foreach ($line in ($text -split "`r`n|`n|`r")) {
                $longestLine = [Math]::Max($longestLine, $line.Length)
            }
            $contentWidth = [Math]::Min(4800, [Math]::Max($MinimumWidth, 480 + ([Math]::Min($longestLine, 60) * 90)))
            $perColumnWidth = $contentWidth / $span
            for ($offset = 0; $offset -lt $span; $offset++) {
                if (($columnIndex + $offset) -lt $columnCount) {
                    $weights[$columnIndex + $offset] = [Math]::Max($weights[$columnIndex + $offset], $perColumnWidth)
                }
            }
            $columnIndex += $span
        }
    ...
    $widths = New-Object 'System.Int32[]' $columnCount
    if (($columnCount * $MinimumWidth) -ge $TotalWidth) {
        $equalWidth = [int][Math]::Floor($TotalWidth / $columnCount)
        for ($index = 0; $index -lt $columnCount; $index++) { $widths[$index] = $equalWidth }
    }
    else {
        $remainingWidth = $TotalWidth - ($columnCount * $MinimumWidth)
        $extras = @($weights | ForEach-Object { [Math]::Max(0.0, $_ - $MinimumWidth) })
        $extrasTotal = ($extras | Measure-Object -Sum).Sum
        for ($index = 0; $index -lt $columnCount; $index++) {
            $extra = if ($extrasTotal -gt 0) {
                [int][Math]::Round($remainingWidth * ($extras[$index] / $extrasTotal))
            }
            else {
                [int][Math]::Floor($remainingWidth / $columnCount)
            }
            $widths[$index] = $MinimumWidth + $extra
        }
    }
    $widths[$columnCount - 1] += $TotalWidth - (($widths | Measure-Object -Sum).Sum)
    return $widths
}

function Set-OpenXmlZipTextEntry {
    param(
        [Parameter(Mandatory)][object]$Archive,
    ...
    $path = $Entry.FullName
    $Entry.Delete()
    $replacement = $Archive.CreateEntry($path)
...
function Set-OpenXmlDocumentLayout {
    param([Parameter(Mandatory)][string]$Path)
    ...
    try {
        ...
        if ($null -eq $documentEntry) { throw 'The generated DOCX is missing word/document.xml.' }
        ...
        $documentXml.LoadXml($documentText)
        ...
        $contentWidth = Get-OpenXmlDocumentContentWidth -Document $documentXml -NamespaceManager $namespaceManager
        foreach ($table in @($documentXml.SelectNodes('//w:tbl', $namespaceManager))) {
            $widths = @(Get-OpenXmlContentBasedColumnWidths -Table $table -NamespaceManager $namespaceManager -TotalWidth $contentWidth)
            if ($widths.Count -eq 0) { continue }

            $tableProperties = Ensure-OpenXmlChildElement -Parent $table -Name 'tblPr' -Document $documentXml -NamespaceManager $namespaceManager
            $tableWidth = Ensure-OpenXmlChildElement -Parent $tableProperties -Name 'tblW' -Document $documentXml -NamespaceManager $namespaceManager
            Set-OpenXmlAttribute -Element $tableWidth -Name 'type' -Value 'dxa' -NamespaceManager $namespaceManager
            Set-OpenXmlAttribute -Element $tableWidth -Name 'w' -Value ([string](($widths | Measure-Object -Sum).Sum)) -NamespaceManager $namespaceManager
            $tableLayout = Ensure-OpenXmlChildElement -Parent $tableProperties -Name 'tblLayout' -Document $documentXml -NamespaceManager $namespaceManager
            Set-OpenXmlAttribute -Element $tableLayout -Name 'type' -Value 'autofit' -NamespaceManager $namespaceManager

            $tableGrid = Get-OpenXmlChildElement -Parent $table -Name 'tblGrid' -NamespaceManager $namespaceManager
            if ($null -eq $tableGrid) {
                $tableGrid = $documentXml.CreateElement('w', 'tblGrid', $wordNamespace)
                [void]$table.InsertAfter($tableGrid, $tableProperties)
            }
            $gridColumns = @($tableGrid.SelectNodes('w:gridCol', $namespaceManager))
            for ($index = $gridColumns.Count; $index -lt $widths.Count; $index++) {
                [void]$tableGrid.AppendChild($documentXml.CreateElement('w', 'gridCol', $wordNamespace))
            }
            $gridColumns = @($tableGrid.SelectNodes('w:gridCol', $namespaceManager))
            for ($index = $gridColumns.Count - 1; $index -ge $widths.Count; $index--) {
                [void]$tableGrid.RemoveChild($gridColumns[$index])
            }
            $gridColumns = @($tableGrid.SelectNodes('w:gridCol', $namespaceManager))
            for ($index = 0; $index -lt $widths.Count; $index++) {
                Set-OpenXmlAttribute -Element $gridColumns[$index] -Name 'w' -Value ([string]$widths[$index]) -NamespaceManager $namespaceManager
            }
            ...
            foreach ($row in @($table.SelectNodes('./w:tr', $namespaceManager))) {
                ...
                foreach ($cell in @($row.SelectNodes('./w:tc', $namespaceManager))) {
                    $span = Get-OpenXmlCellGridSpan -Cell $cell -NamespaceManager $namespaceManager
                    $preferredWidth = 0
                    for ($offset = 0; $offset -lt $span -and ($columnIndex + $offset) -lt $widths.Count; $offset++) {
                        $preferredWidth += $widths[$columnIndex + $offset]
                    }
                    $columnIndex += $span
                    $cellProperties = Ensure-OpenXmlChildElement -Parent $cell -Name 'tcPr' -Document $documentXml -NamespaceManager $namespaceManager
                    $cellWidth = Ensure-OpenXmlChildElement -Parent $cellProperties -Name 'tcW' -Document $documentXml -NamespaceManager $namespaceManager
                    Set-OpenXmlAttribute -Element $cellWidth -Name 'type' -Value 'dxa' -NamespaceManager $namespaceManager
                    Set-OpenXmlAttribute -Element $cellWidth -Name 'w' -Value ([string]$preferredWidth) -NamespaceManager $namespaceManager
                    $cellMargins = Ensure-OpenXmlChildElement -Parent $cellProperties -Name 'tcMar' -Document $documentXml -NamespaceManager $namespaceManager
                    foreach ($side in @('left', 'right')) {
                        $margin = Ensure-OpenXmlChildElement -Parent $cellMargins -Name $side -Document $documentXml -NamespaceManager $namespaceManager
                        Set-OpenXmlAttribute -Element $margin -Name 'w' -Value '120' -NamespaceManager $namespaceManager
                        Set-OpenXmlAttribute -Element $margin -Name 'type' -Value 'dxa' -NamespaceManager $namespaceManager
                    }
                }
            }
        }
        Set-OpenXmlZipTextEntry -Archive $archive -Entry $documentEntry -Content $documentXml.OuterXml

        foreach ($footerEntry in @($archive.Entries | Where-Object { $_.FullName -match '^word/footer\d+\.xml$' })) {
            $reader = New-Object IO.StreamReader($footerEntry.Open())
            try { $footerText = $reader.ReadToEnd() }
            finally { $reader.Dispose() }
            $footerXml = New-Object System.Xml.XmlDocument
            $footerXml.PreserveWhitespace = $false
            $footerXml.LoadXml($footerText)
            $footerNamespaces = New-Object System.Xml.XmlNamespaceManager($footerXml.NameTable)
            $footerNamespaces.AddNamespace('w', $wordNamespace)
            foreach ($run in @($footerXml.SelectNodes('//w:r', $footerNamespaces))) {
                $runProperties = Get-OpenXmlChildElement -Parent $run -Name 'rPr' -NamespaceManager $footerNamespaces
                if ($null -eq $runProperties) {
                    $runProperties = $footerXml.CreateElement('w', 'rPr', $wordNamespace)
                    if ($null -ne $run.FirstChild) { [void]$run.InsertBefore($runProperties, $run.FirstChild) }
                    else { [void]$run.AppendChild($runProperties) }
                }
                foreach ($sizeName in @('sz', 'szCs')) {
                    $size = Ensure-OpenXmlChildElement -Parent $runProperties -Name $sizeName -Document $footerXml -NamespaceManager $footerNamespaces
                    Set-OpenXmlAttribute -Element $size -Name 'val' -Value '17' -NamespaceManager $footerNamespaces
                }
            }
            Set-OpenXmlZipTextEntry -Archive $archive -Entry $footerEntry -Content $footerXml.OuterXml
        }
    }
    finally {
        $archive.Dispose()
...
function New-OpenXmlHealthReport {
    ...
    $logoPath = if ($LogoPath) { $LogoPath } else { Resolve-ReportLogoPath }
    if (-not (Test-Path $logoPath)) { throw "The WinslowTG logo is missing: $logoPath" }
    ...
    foreach ($item in @(
        ...
        [void]$body.Add((New-OpenXmlParagraph -Text $item[0] -Style 'Heading2' -Before 160 -After 100 -Bold -Color '404040' -Size 13 -KeepNext))
    ...
    if ($remoteSupportRows.Count -gt 0) {
        ...
        [void]$body.Add((New-OpenXmlParagraph -Text 'Registration' -Style 'Heading2' -Before 160 -After 100 -Bold -Color '404040' -Size 13 -KeepNext))
    ...
    $systemSectionRecords = @($systemSections | ForEach-Object {
        ...
        if ($_[0] -eq 'Summary') { $recordArguments.ExcludeColumns = @('Health') }
...
"@
    $headerXml = @"
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<w:hdr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:wp="http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing" xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:pic="http://schemas.openxmlformats.org/drawingml/2006/picture"><w:tbl><w:tblPr><w:tblW w:w="10800" w:type="dxa"/><w:tblLayout w:type="fixed"/></w:tblPr><w:tblGrid><w:gridCol w:w="2300"/><w:gridCol w:w="8500"/></w:tblGrid><w:tr><w:tc><w:tcPr><w:tcW w:w="2300" w:type="dxa"/><w:vAlign w:val="center"/><w:tcMar><w:top w:w="0" w:type="dxa"/><w:bottom w:w="40" w:type="dxa"/><w:start w:w="0" w:type="dxa"/><w:end w:w="100" w:type="dxa"/></w:tcMar><w:tcBorders><w:top w:val="nil"/><w:left w:val="nil"/><w:right w:val="nil"/><w:bottom w:val="single" w:sz="6" w:color="005F9E"/></w:tcBorders></w:tcPr><w:p><w:pPr><w:jc w:val="left"/><w:spacing w:before="0" w:after="0"/></w:pPr>$(New-OpenXmlLogoRun)</w:p></w:tc><w:tc><w:tcPr><w:tcW w:w="8500" w:type="dxa"/><w:vAlign w:val="center"/><w:tcMar><w:top w:w="0" w:type="dxa"/><w:bottom w:w="40" w:type="dxa"/><w:start w:w="100" w:type="dxa"/><w:end w:w="0" w:type="dxa"/></w:tcMar><w:tcBorders><w:top w:val="nil"/><w:left w:val="nil"/><w:right w:val="nil"/><w:bottom w:val="single" w:sz="6" w:color="005F9E"/></w:tcBorders></w:tcPr><w:p><w:pPr><w:jc w:val="right"/><w:spacing w:before="0" w:after="0"/></w:pPr>$(New-OpenXmlRun -Text $headerText -Size 8 -Color '404040')</w:p></w:tc></w:tr></w:tbl></w:hdr>
"@
    $footerXml = @"
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<w:ftr xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:p><w:pPr><w:tabs><w:tab w:val="center" w:pos="5400"/><w:tab w:val="right" w:pos="10800"/></w:tabs><w:pBdr><w:top w:val="single" w:sz="6" w:space="4" w:color="005F9E"/></w:pBdr><w:spacing w:before="80"/></w:pPr>$(New-OpenXmlRun -Text 'Confidential' -Size 8.5 -Color '506675' -Italic)<w:r><w:tab/></w:r>$(New-OpenXmlRun -Text "$([char]0x00A9)2026 Winslow Tech Group. All Right Reserved" -Size 8.5 -Color '506675')<w:r><w:tab/></w:r>$(New-OpenXmlRun -Text 'Page ' -Size 8.5 -Color '506675')<w:fldSimple w:instr="PAGE"><w:r><w:rPr><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/><w:sz w:val="17"/><w:color w:val="506675"/></w:rPr><w:t>1</w:t></w:r></w:fldSimple>$(New-OpenXmlRun -Text ' of ' -Size 8.5 -Color '506675')<w:fldSimple w:instr="NUMPAGES"><w:r><w:rPr><w:rFonts w:ascii="Calibri" w:hAnsi="Calibri"/><w:sz w:val="17"/><w:color w:val="506675"/></w:rPr><w:t>1</w:t></w:r></w:fldSimple></w:p></w:ftr>
"@
    $contentTypes = @"
<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
...
"@
    ...
    Set-OpenXmlDocumentLayout -Path $resolved
    return $resolved
}
...
function New-WordHealthReport {
    ...
    try {
        ...
        foreach ($item in $systemSections) {
            $initialRecordArguments = @{
                Data = $Data
                PropertyName = $item[1]
                PropertyMap = [bool]$item[2]
            }
            if ($item[0] -eq 'Summary') { $initialRecordArguments.ExcludeColumns = @('Health') }
            if (@(Get-ReportRecords @initialRecordArguments).Count -gt 0) {
        ...
        if ($hasSystemInformation) {
            ...
            foreach ($item in $systemSections) {
                ...
                if ($item[0] -eq 'Summary') { $recordArguments.ExcludeColumns = @('Health') }
